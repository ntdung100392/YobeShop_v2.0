    ...
    for($i=1;$i<=$this->config->get('yobepayment_number_method');$i++)
    {
        ...
        if (isset($this->request->post['yobepayment_image'.$i])) {
        ...
        if (isset($this->request->post['yobepayment_image'.$i]) && is_file(DIR_IMAGE . $this->request->post['yobepayment_image'.$i])) {
            $data['thumb'.$i] = $this->model_tool_image->resize($this->request->post['yobepayment_image'.$i], 100, 100);
        } elseif ($this->config->get('yobepayment_image'.$i) && is_file(DIR_IMAGE . $this->config->get('yobepayment_image'.$i))) {
